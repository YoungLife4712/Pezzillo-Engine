...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "FUNNY BOTPLAY", 32);
	...
	override public function update(elapsed:Float)
	{
		...
		if (songMisses >= 10 && ClientPrefs.gamerMode)	
			health = 0;

		if(ClientPrefs.gamerMode) 
			{
				opponentStrums.members[0].x = -500;
				opponentStrums.members[1].x = -500;
				opponentStrums.members[2].x = -500;
				opponentStrums.members[3].x = -500;
			};
		...
		if(ratingName == '?') {
			scoreTxt.text = 'Score: ' + songScore + ' | Health: ' + health + '% ' + ' | Skill Issues: ' + songMisses + ' | Rating: ' + ratingName;
		} else {
			scoreTxt.text = 'Score: ' + songScore + ' | Health: ' + health + '% ' + ' | Skill Issues: ' + songMisses + ' | Rating: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - ' + ratingFC;//peeps wanted no integer rating
		}
	...
	public function finishSong(?ignoreNoteOffset:Bool = false):Void
	{
		...
		if(ratingPercent <= 0.85 && ClientPrefs.gamerMode)
			health = 0;
